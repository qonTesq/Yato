...
module.exports = {
	...
	run: async ({ client, guild, member, respond }, arrayArgs, args) => {
		...
		if (args.user && args.user !== member.user.id) {
			...
			str = `**${member.user.username}** made ${user} **CRY!**${args.reason ? `, ${args.reason}` : ''}`;
		} else {
			str = `${member} is **Crying **! Aww..`;
		}

		const image = await axios.get('https://waifu.pics/api/sfw/cry').then(res => res.data).catch(err => {
			console.log(err);
