...
module.exports = {
	...
	run: async ({ client, guild, member, respond }, arrayArgs, args) => {
		...
		const links = ['https://waifu.pics/api/sfw/slap', 'https://nekos.life/api/v2/img/slap'];
		let str;

		if (args.user && args.user !== member.user.id) {
			const user = guild.members.cache.get(args.user);
			str = `**${member.user.username}** *slaps* ${user}${args.reason ? `, ${args.reason}` : ''}`;
		} else {
			return respond(`Just why?`);
		}
